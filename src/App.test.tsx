...
beforeEach(() => {
  ...
  vi.spyOn(GreenApiTelegram.prototype, 'resolvePhone').mockResolvedValue({ chatId });
  vi.spyOn(GreenApiTelegram.prototype, 'sendMessage').mockResolvedValue({ idMessage: 'sent-1' });
  vi.spyOn(GreenApiTelegram.prototype, 'deleteNotification').mockResolvedValue(true);
  vi.spyOn(GreenApiTelegram.prototype, 'receiveNotification').mockImplementation((signal) => new Promise((resolve, reject) => {
...
function draft() {
  return screen.getByRole('textbox', { name: 'Сообщение' }) as HTMLTextAreaElement;
}
...
describe('основной сценарий Telegram', () => {
  it('загружает существующие чаты после входа и историю без потери новых сообщений и черновика', async () => {
    vi.mocked(GreenApiTelegram.prototype.getChats).mockResolvedValue([{ id: chatId, title: 'Анна', phone }]);
    let finishHistory!: (messages: HistoryMessage[]) => void;
    vi.mocked(GreenApiTelegram.prototype.getChatHistory).mockImplementationOnce(() => new Promise((resolve) => { finishHistory = resolve; }));
    const user = await connect(true);
    const navigation = within(screen.getByRole('navigation', { name: 'Список чатов' }));
    ...
    await waitFor(() => expect(GreenApiTelegram.prototype.getChatHistory).toHaveBeenCalledWith(chatId, expect.any(AbortSignal)));
    expect(GreenApiTelegram.prototype.resolvePhone).not.toHaveBeenCalled();
    await user.type(draft(), 'Мой черновик');
    await act(async () => receive({ receiptId: 17, body: {
      typeWebhook: 'incomingMessageReceived', idMessage: 'reply-1', timestamp: 1763115112,
      senderData: { chatId, senderName: 'Анна' },
      messageData: { typeMessage: 'textMessage', textMessageData: { textMessage: 'Новый ответ' } },
    } }));
    await act(async () => finishHistory([
      { id: 'old-1', chatId, text: 'Предыдущее сообщение', timestamp: 1763115000000, senderName: 'Вы', direction: 'outgoing', status: 'sent' },
      { id: 'reply-1', chatId, text: 'Новый ответ', timestamp: 1763115112000, senderName: 'Анна', direction: 'incoming', status: 'sent' },
    ]));
  ...
  it('показывает сбой загрузки чатов и восстанавливает список после повтора', async () => {
    vi.mocked(GreenApiTelegram.prototype.getChats)
      .mockRejectedValueOnce(new GreenApiError('Не удалось загрузить чаты.', 503))
      .mockResolvedValueOnce([{ id: chatId, title: 'Анна' }]);
    const user = await connect();
    expect((await screen.findByRole('alert')).textContent).toContain('Не удалось загрузить чаты');
    expect(screen.queryByText('Нет чатов')).toBeNull();
    await user.click(screen.getByRole('button', { name: 'Повторить загрузку чатов' }));
    expect(await within(screen.getByRole('navigation')).findByRole('button', { name: 'Анна' })).toBeTruthy();
    expect(screen.queryByRole('alert')).toBeNull();
  });

  it('позволяет повторить загрузку истории после сбоя', async () => {
    vi.mocked(GreenApiTelegram.prototype.getChats).mockResolvedValue([{ id: chatId, title: 'Анна' }]);
    vi.mocked(GreenApiTelegram.prototype.getChatHistory)
      .mockRejectedValueOnce(new GreenApiError('Не удалось загрузить сообщения.', 503))
      .mockResolvedValueOnce([{ id: 'old-1', chatId, text: 'История чата', timestamp: 1763115000000, senderName: 'Анна', direction: 'incoming', status: 'sent' }]);
    const user = await connect();
    await user.click(await within(screen.getByRole('navigation')).findByRole('button', { name: 'Анна' }));
    await user.click(await screen.findByRole('button', { name: 'Повторить загрузку сообщений' }));
    expect(await within(screen.getByRole('log')).findByText('История чата', {}, { timeout: 2000 })).toBeTruthy();
    expect(screen.queryByRole('alert')).toBeNull();
  });
  ...
  it('подключается, создаёт чат по телефону, отправляет текст и получает ответ', async () => {
    const user = await connect();
    await openChat(user);
    expect(GreenApiTelegram.prototype.resolvePhone).toHaveBeenCalledWith(phone, expect.any(AbortSignal));
    await user.type(draft(), 'Привет, Анна');
    await user.click(screen.getByRole('button', { name: 'Отправить' }));
    expect(GreenApiTelegram.prototype.sendMessage).toHaveBeenCalledWith(chatId, 'Привет, Анна', expect.any(AbortSignal));
    ...
    expect(messages.getAllByRole('listitem')).toHaveLength(2);
    expect(within(screen.getByRole('navigation', { name: 'Список чатов' })).getAllByRole('button')).toHaveLength(1);
    expect(GreenApiTelegram.prototype.deleteNotification).toHaveBeenCalledWith(17, expect.any(AbortSignal));
  });
  ...
  it('сохраняет раннюю доставку и обновляет галочки до прочтения без отката', async () => {
    ...
    vi.mocked(GreenApiTelegram.prototype.sendMessage).mockImplementationOnce(() => new Promise((resolve) => { finishSend = resolve; }));
    const user = await connect();
    await openChat(user);
    ...
    const status = (value: string, receiptId: number): Notification => ({ receiptId, body: {
      typeWebhook: 'outgoingMessageStatus', chatId, idMessage: 'sent-1', status: value,
    } });
    await act(async () => receive(status('delivered', 18)));
    await act(async () => finishSend({ idMessage: 'sent-1' }));
    expect(log.querySelector('.status-delivered .message-check')).not.toBeNull();
  ...
  it.each(['отказ API', 'неавторизованный инстанс'])('оставляет форму доступной: %s', async (failure) => {
    ...
    render(<App />);
    await credentials(user);
    expect((await screen.findByRole('alert')).textContent).toMatch(/Доступ запрещён|Авторизуйте инстанс/);
    ...
    expect(GreenApiTelegram.prototype.receiveNotification).not.toHaveBeenCalled();
  });

  it('объясняет настройку HTTP-очереди и начинает получение после повторной проверки', async () => {
    vi.mocked(GreenApiTelegram.prototype.getSettings).mockResolvedValueOnce({ incomingWebhook: 'no', webhookUrl: 'https://example.com/hook' });
    const user = await connect();
    expect(screen.getByText(/включите входящие уведомления и очистите адрес webhook/)).toBeTruthy();
    expect(GreenApiTelegram.prototype.receiveNotification).not.toHaveBeenCalled();
    ...
    expect(GreenApiTelegram.prototype.receiveNotification).toHaveBeenCalledWith(expect.any(AbortSignal));
    expect(screen.queryByText(/включите входящие уведомления и очистите адрес webhook/)).toBeNull();
  });

  it('показывает ошибку поиска номера и позволяет исправить получателя', async () => {
    vi.mocked(GreenApiTelegram.prototype.resolvePhone).mockRejectedValueOnce(new GreenApiError('Аккаунт не найден.'));
    const user = await connect();
    await user.type(screen.getByLabelText('Номер телефона'), phone);
    await user.click(screen.getByRole('button', { name: 'Новый чат' }));
    expect((await screen.findByRole('alert')).textContent).toContain('Аккаунт не найден');
    expect((screen.getByLabelText('Номер телефона') as HTMLInputElement).value).toBe(phone);
    expect((screen.getByRole('button', { name: 'Новый чат' }) as HTMLButtonElement).disabled).toBe(false);
  ...
  it('сохраняет черновик после ошибки отправки без автоматического повтора', async () => {
    vi.mocked(GreenApiTelegram.prototype.sendMessage).mockRejectedValueOnce(new GreenApiError('Сервис недоступен.', 503));
    const user = await connect();
    await openChat(user);
  ...
  it('отправляет по Enter, сохраняет переносы, форматирует текст и экранирует HTML', async () => {
    ...
    expect(GreenApiTelegram.prototype.sendMessage).not.toHaveBeenCalled();
    await user.keyboard('{Enter}');
    expect(GreenApiTelegram.prototype.sendMessage).toHaveBeenCalledWith(chatId, '*Привет*\n<b>мир</b>', expect.any(AbortSignal));
    const log = screen.getByRole('log', { name: 'Сообщения' });
  ...
  it('при отключении отменяет отправку и получение, очищает данные входа', async () => {
    vi.mocked(GreenApiTelegram.prototype.sendMessage).mockImplementation((_chatId, _text, signal) => new Promise((_, reject) => {
    ...
    await user.click(screen.getByRole('button', { name: 'Отправить' }));
    const sendSignal = vi.mocked(GreenApiTelegram.prototype.sendMessage).mock.calls[0]?.[2];
    const receiveSignal = vi.mocked(GreenApiTelegram.prototype.receiveNotification).mock.calls[0]?.[0];
