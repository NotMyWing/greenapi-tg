...
function invalidResponse(): GreenApiError {
  return new GreenApiError('GREEN-API вернул неожиданный ответ. Повторите попытку.');
}
...
function apiError(status: number, response: string): GreenApiError {
  ...
  let message = 'GREEN-API отклонил запрос. Проверьте данные и состояние инстанса.';
  if (status === 401 || status === 403) message = 'Доступ запрещён. Проверьте idInstance, apiTokenInstance и apiUrl.';
  else if (status === 429) message = 'Слишком много запросов. Подождите немного и повторите попытку.';
  else if (status === 469 || detail.includes('rate_limit')) message = 'Telegram временно ограничил поиск номеров. Попробуйте через несколько часов.';
  else if (detail.includes('webhook url is set')) message = 'Очистите webhookUrl в настройках инстанса и подождите около минуты.';
  else if (detail.includes('not authorized') || detail.includes('starting')) message = 'Авторизуйте инстанс Telegram в кабинете GREEN-API и повторите вход.';
  else if (status === 404) message = 'Инстанс не найден. Проверьте apiUrl и idInstance.';
  else if (status >= 500) message = 'GREEN-API временно недоступен. Попробуйте позже.';
...
function normalizeCredentials(credentials: Credentials): Credentials {
  ...
  if (!/^\d+$/.test(idInstance)) throw new GreenApiError('idInstance должен содержать только цифры.');
  if (!/^[\w-]+$/.test(apiTokenInstance)) throw new GreenApiError('Укажите корректный apiTokenInstance из личного кабинета.');
...
export class GreenApiTelegram {
  ...
  async #request(
    ...
    try {
      ...
      if (method === 'receiveNotification' && controller.signal.aborted) return null;
      const message = controller.signal.aborted ? 'GREEN-API не ответил вовремя.' : 'Не удалось связаться с GREEN-API. Проверьте интернет и apiUrl.';
      throw new GreenApiError(message + (method === 'sendMessage'
  ...
  async getState(signal?: AbortSignal): Promise<string> {
    const data = await this.#request('getStateInstance', signal, undefined, '', 5_000);
  ...
  async getSettings(signal?: AbortSignal): Promise<InstanceSettings> {
    ...
    if (!isRecord(data) || typeof data.webhookUrl !== 'string' || (data.incomingWebhook !== 'yes' && data.incomingWebhook !== 'no')) throw invalidResponse();
    if (data.typeInstance !== undefined && data.typeInstance !== 'telegram') throw new GreenApiError('Выберите инстанс Telegram в личном кабинете GREEN-API.');
  ...
  async getChats(signal?: AbortSignal): Promise<TelegramChat[]> {
    ...
    return data.map((chat: unknown) => {
      if (!isRecord(chat) || !isText(chat.chatId) || !/^-?[1-9]\d*$/.test(chat.chatId)) throw invalidResponse();
      ...
      return {
        ...
        ...(typeof phone === 'number' && Number.isSafeInteger(phone) && phone > 0 ? { phone: String(phone) } : {}),
      };
    });
  }

  /** Берём текст из последних 100 записей, от старых к новым. */
  async getChatHistory(chatId: string, signal?: AbortSignal): Promise<HistoryMessage[]> {
    if (!/^-?[1-9]\d*$/.test(chatId)) throw new GreenApiError('Некорректный идентификатор чата Telegram.');
    const data = await this.#request('getChatHistory', signal, { chatId, count: 100 });
    if (!Array.isArray(data)) throw invalidResponse();
    const messages: HistoryMessage[] = [];
    for (const item of data) {
      if (!isRecord(item)) throw invalidResponse();
      if (item.isDeleted === true || item.typeMessage !== 'textMessage') continue;
      if ((item.type !== 'incoming' && item.type !== 'outgoing') || !isText(item.idMessage) ||
        item.chatId !== chatId || !isText(item.textMessage) || typeof item.timestamp !== 'number' ||
        !Number.isSafeInteger(item.timestamp) || item.timestamp < 0 || item.timestamp > 8_640_000_000_000) throw invalidResponse();
      messages.push({
        id: item.idMessage, chatId, text: item.textMessage, timestamp: item.timestamp * 1000,
        senderName: isText(item.senderName) ? item.senderName : chatId,
        direction: item.type,
        status: item.statusMessage === 'queued' || item.statusMessage === 'failed' ||
          item.statusMessage === 'delivered' || item.statusMessage === 'read' ? item.statusMessage : 'sent',
      });
    }
  ...
  async resolvePhone(phone: string, signal?: AbortSignal): Promise<{ chatId: string }> {
    const data = await this.#request('checkAccount', signal, { phoneNumber: Number(normalizePhone(phone)) });
    if (!isRecord(data) || typeof data.exist !== 'boolean') throw invalidResponse();
    if (!data.exist) throw new GreenApiError('Аккаунт Telegram не найден или поиск по номеру ограничен настройками приватности.');
  ...
  async sendMessage(chatId: string, message: string, signal?: AbortSignal): Promise<{ idMessage: string }> {
    if (!/^-?[1-9]\d*$/.test(chatId)) throw new GreenApiError('Некорректный идентификатор чата Telegram.');
    ...
    const data = await this.#request('sendMessage', signal, { chatId, message });
  ...
  async receiveNotification(signal?: AbortSignal): Promise<Notification | null> {
    const data = await this.#request('receiveNotification', signal, undefined, '?receiveTimeout=30', 40_000);
    if (data === null) return null;
    if (!isRecord(data) || typeof data.receiptId !== 'number' || !Number.isSafeInteger(data.receiptId) || data.receiptId < 0 || !('body' in data)) throw invalidResponse();
  ...
  async deleteNotification(receiptId: number, signal?: AbortSignal): Promise<boolean> {
    ...
    const data = await this.#request('deleteNotification', signal, undefined, `/${receiptId}`, 20_000, 'DELETE');
...
export function parseTextMessage(body: unknown): IncomingTextMessage | null {
  if (!isRecord(body) || body.typeWebhook !== 'incomingMessageReceived' || !isRecord(body.senderData) || !isRecord(body.messageData)) return null;
  const { senderData, messageData } = body;
  if (!isText(body.idMessage) || !isText(senderData.chatId) || !/^-?[1-9]\d*$/.test(senderData.chatId) ||
    typeof body.timestamp !== 'number' || !Number.isFinite(body.timestamp) || body.timestamp < 0 || body.timestamp > 8_640_000_000_000) return null;
  let text: unknown;
  if (messageData.typeMessage === 'textMessage' && isRecord(messageData.textMessageData)) text = messageData.textMessageData.textMessage;
  if (messageData.typeMessage === 'extendedTextMessage' && isRecord(messageData.extendedTextMessageData)) text = messageData.extendedTextMessageData.text;
...
export function parseOutgoingStatus(body: unknown): OutgoingMessageStatus | null {
  if (!isRecord(body) || body.typeWebhook !== 'outgoingMessageStatus' || !isText(body.chatId) ||
    !/^-?[1-9]\d*$/.test(body.chatId) || !isText(body.idMessage)) return null;
  ...
  return {
    chatId: body.chatId, messageId: body.idMessage, status,
    ...(status === 'failed' ? { error: 'Telegram не смог отправить сообщение. Проверьте чат перед повторной отправкой.' } : {}),
  };
}
