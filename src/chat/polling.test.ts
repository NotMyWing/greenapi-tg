...
function setup() {
  ...
  const client = {
    receiveNotification: vi.fn<GreenApiTelegram['receiveNotification']>().mockResolvedValue(null),
    deleteNotification: vi.fn<GreenApiTelegram['deleteNotification']>().mockResolvedValue(true),
  };
  const onMessage = vi.fn();
  const onStatus = vi.fn();
  const onError = vi.fn();
  return { client, controller, onMessage, onStatus, onError,
    start: () => pollNotifications(client, { signal: controller.signal, onMessage, onStatus, onError }),
  };
}
...
describe('очередь уведомлений', () => {
  it('тихо продолжает опрос после HTTP 408 и локального таймаута, сохраняя ошибки удаления', async () => {
    ...
    const timeout: typeof fetch = (_url, init) => new Promise((_, reject) => {
    ...
    const client = new GreenApiTelegram({ apiUrl: '', idInstance: '4100000000', apiTokenInstance: 'test-token' });
    const done = pollNotifications(client, { signal: controller.signal, onMessage, onStatus, onError });
    await vi.advanceTimersByTimeAsync(43_000);
    expect(fetchMock).toHaveBeenCalledTimes(2);
    expect(onError.mock.calls).toEqual([[null], [null]]);
    await vi.advanceTimersByTimeAsync(1000);
    expect(onMessage).toHaveBeenCalledWith(expect.objectContaining({ id: 'reply-1' }));
    await vi.advanceTimersByTimeAsync(20_000);
    expect(onError).toHaveBeenLastCalledWith('GREEN-API не ответил вовремя. Попробуйте ещё раз.');
    await vi.advanceTimersByTimeAsync(3000);
    expect(fetchMock).toHaveBeenCalledTimes(5);
    expect(fetchMock.mock.calls[4]?.[0]).toContain('/deleteNotification/test-token/17');
  ...
  it('обрабатывает текст и статусы до удаления и подтверждает неизвестные события', async () => {
    ...
    client.receiveNotification.mockResolvedValueOnce(notification)
      .mockResolvedValueOnce({ receiptId: 18, body: { typeWebhook: 'outgoingMessageStatus', chatId: '123456', idMessage: 'sent-1', status: 'read' } })
      .mockResolvedValueOnce({ receiptId: 19, body: null });
    client.deleteNotification.mockImplementationOnce(async () => {
      expect(onMessage).toHaveBeenCalledWith(expect.objectContaining({ id: 'reply-1', text: 'Привет!' }));
      return true;
    }).mockImplementationOnce(async () => {
      expect(onStatus).toHaveBeenCalledWith({ chatId: '123456', messageId: 'sent-1', status: 'read' });
      return true;
    });
    const done = start();
    await vi.advanceTimersByTimeAsync(0);
    expect(client.deleteNotification).toHaveBeenCalledWith(17, controller.signal);
    expect(client.receiveNotification).toHaveBeenCalledTimes(1);
    await vi.advanceTimersByTimeAsync(1000);
    expect(client.deleteNotification).toHaveBeenLastCalledWith(18, controller.signal);
    await vi.advanceTimersByTimeAsync(1000);
    expect(client.deleteNotification).toHaveBeenLastCalledWith(19, controller.signal);
    expect(onMessage).toHaveBeenCalledTimes(1);
  ...
  it('повторяет неудачное удаление того же уведомления без повторной обработки или получения', async () => {
    ...
    client.receiveNotification.mockResolvedValueOnce(notification);
    client.deleteNotification.mockRejectedValueOnce(new GreenApiError('Сервис недоступен.', 503));
    const done = start();
    ...
    expect(client.deleteNotification).toHaveBeenLastCalledWith(17, controller.signal);
    expect(client.receiveNotification).toHaveBeenCalledTimes(1);
    expect(onMessage).toHaveBeenCalledTimes(1);
    controller.abort();
    await done;
  ...
  it('после 429 ждёт 30 секунд, а отмена прерывает активный запрос', async () => {
    ...
    expect(client.receiveNotification).toHaveBeenLastCalledWith(controller.signal);
  ...
  it('останавливается при отказе доступа и показывает причину', async () => {
    ...
    client.receiveNotification.mockRejectedValueOnce(new GreenApiError('Доступ запрещён.', 401));
    await start();
