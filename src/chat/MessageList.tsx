...
export function MessageList({ messages, chatId, chatTitle, loading }: MessageListProps) {
  ...
  return (
    ...
    >
      {loading && <p className="chat-empty" role="status">Загрузка сообщений…</p>}
      {messages.length === 0 && !loading && <p className="chat-empty">Нет сообщений</p>}
      <ol>
        {messages.map((message, index) => {
          ...
          const name = message.direction === 'outgoing' ? 'Вы' : message.senderName || chatTitle;
          ...
          const groupStart = !previous || previous.direction !== message.direction ||
            ...
            dateFormat.format(previous.timestamp) !== date || message.timestamp - previous.timestamp > 300_000;
          const groupEnd = !next || next.direction !== message.direction ||
            next.senderName !== message.senderName ||
            dateFormat.format(next.timestamp) !== date || next.timestamp - message.timestamp > 300_000;
          return (
            <li key={message.id} className={`message-row ${message.direction}${groupEnd ? ' group-end' : ''}`}>
              {(!previous || dateFormat.format(previous.timestamp) !== date) && <div className="message-date">{date}</div>}
              <div className="message-content">
                <span className="message-avatar">
                  {groupEnd && <Avatar id={authorId} name={name} size={32} />}
                </span>
                <div className={`message-bubble ${message.status === 'failed' ? 'message-failed' : ''}`}>
                  {groupStart && name && !/^[+\d-]+$/.test(name) && <div className="message-author">{name}</div>}
                  <div className="message-text">
                    <FormattedText text={message.text} />
                    <span className="message-meta">
                      <time dateTime={new Date(message.timestamp).toISOString()}>{timeFormat.format(message.timestamp)}</time>
