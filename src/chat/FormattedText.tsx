...
function safeHref(value: string): string | null {
  try {
    ...
    return /^(?:https?|tg):$/.test(url.protocol) && !url.username && !url.password ? url.href : null;
  } catch {
    return null;
  }
}
...
export function FormattedText({ text }: { text: string }) {
  ...
  while ((match = pattern.exec(text))) {
    ...
    if (match[1]) content = <code>{match[1]}</code>;
      ...
      const address = match[3] ? candidate : candidate.replace(/[.,!?;:]+$/, '');
      const href = safeHref(address);
      if (closed && href) content = <>
        <a href={href} target="_blank" rel="noopener noreferrer">{match[2] ?? address}</a>
        {match[4] ? raw.slice(address.length) : ''}
      </>;
    } else {
      const value = match[5] ?? match[6] ?? match[7] ?? match[8] ?? match[9];
      if (value && value.trim() === value) {
        if (match[5] || match[6] || match[8]) content = <strong>{value}</strong>;
  ...
  return <span className="message-paragraph">{parts}{text.slice(position)}</span>;
}
